...
pub struct RewriteConfig {
    /// Optional callback invoked on warnings (e.g., invalid exclude patterns).
    /// If `None`, warnings are silently ignored.
    pub on_warning: Option<fn(&str)>,
}

/// Rewrite a command to its RTK equivalent with custom configuration.
///
/// This is the full-featured variant that accepts a [`RewriteConfig`]
/// for controlling warning callbacks. See [`rewrite_command`] for the
/// convenience wrapper.
pub fn rewrite_command_with_config(
    ...
    let compiled = compile_exclude_patterns(excluded, config);
    ...
    rewrite_compound(trimmed, &compiled, &normalized_prefixes, config)
}
    ...
    for tok in &tokens {
        ...
        match tok.kind {
            TokenKind::Operator => {
                ...
                let rewritten = rewrite_segment(seg, excluded, transparent_prefixes, config)
            ...
            TokenKind::Pipe => {
                ...
                let rewritten = if is_pipe_incompatible {
                    ...
                    rewrite_segment(seg, excluded, transparent_prefixes, config)
                        .unwrap_or_else(|| seg.to_string())
                };
                if rewritten != seg {
                    any_changed = true;
...
fn compile_exclude_patterns(patterns: &[String], config: &RewriteConfig) -> Vec<ExcludePattern> {
    patterns
        .iter()
        .filter_map(|pattern| {
            let trimmed = pattern.trim();
            if trimmed.is_empty() || trimmed == "^" {
                if let Some(cb) = config.on_warning {
                    cb(&format!(
...
fn rewrite_segment(
    ...
    rewrite_segment_inner(seg, excluded, transparent_prefixes, 0, config)
}
...
fn rewrite_segment_inner(
    ...
    for &prefix in SHELL_PREFIX_BUILTINS {
        if let Some(rest) = strip_word_prefix(trimmed, prefix) {
            ...
            return rewrite_segment_inner(rest, excluded, transparent_prefixes, depth + 1, config)
